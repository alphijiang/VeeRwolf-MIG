...
assign {dmi_reg_addr, dmi_reg_wdata, wr_en, rd_en} = dtmcs_r;
   ...
   // Three-stage request synchronizers from the BSCAN DTMCS clock domain to
   // the VeeR core clock domain. The address and data payload remain stable
   // while either request propagates through this chain.
   (* ASYNC_REG = "TRUE", SHREG_EXTRACT = "NO" *) reg [2:0] rden, wren;
